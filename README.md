...
Training examples carry no system message at all (see [The system prompt](#the-system-prompt-trigger-or-baked-in) below
for why). The BQL reference that would otherwise have to go in a system prompt is kept as `LONG_REFERENCE` in
[`schema.py`](src/bql_lora/schema.py), only for prompting a *non*-fine-tuned model as a baseline to compare against.
...
- [`src/bql_lora/schema.py`](src/bql_lora/schema.py): `LONG_REFERENCE` (the eval-baseline prompt) and the schema-mode renderers.
...
- [`scripts/check_system_prompt_sensitivity.py`](scripts/check_system_prompt_sensitivity.py): a regression check for a
  newly retrained model — does it still answer in BQL with and without the Modelfile's `SYSTEM` line present?
- [`data/`](data): the training set.
...
`meta` is for filtering and inspection; most trainers only read `messages`. Examples are short: roughly 75-250 tokens
(estimated from character counts). `max_seq_length=1024` fits everything and 2048 leaves margin; there is no need for
a model's maximum context (e.g. 32768), which needs far more VRAM.

## The system prompt: trigger or baked in?

There isn't one, and that was a deliberate finding, not the starting design. A constant system prompt is the easiest
feature for a fine-tune to key on instead of learning the behaviour for any input: an earlier version of this dataset
put a ~100-token BQL system prompt in every example, and a model trained on it did BQL only when that exact prompt was
present. Measured with [`scripts/check_system_prompt_sensitivity.py`](scripts/check_system_prompt_sensitivity.py) on 8
questions:
...
That works while a Modelfile supplies the exact prompt, and breaks the moment a client sends its own system message
(Ollama then ignores the Modelfile's `SYSTEM`). Training on the same examples with no system message at all — what
`data/` now holds — pushes the behaviour into the weights instead (it also cuts an example from about 255 to about 75
tokens). Qwen's chat template fills in its own default line for a conversation with none, so that is effectively what
the model trains on. Measured the same way after retraining on it:

| System message at inference | Answers containing a `sql` block |
| --- | --- |
| Qwen's stock line | 8 of 8 |
| none | 7 of 8 (the one miss: a plain "how are you" correctly got a plain reply, not a forced query) |
| an unrelated custom system prompt | 8 of 8 |

So `ollama/Modelfile` bakes in Qwen's stock line (`SYSTEM` has to be set explicitly: the stripped-down template this
repo uses has no built-in fallback like Qwen's own template does), and `scripts/train.py` trains on `data/` with no
system message, unconditionally. There is no longer a choice to make here.
...
- the **first training example exactly as the model sees it**, including the system line Qwen's chat template fills in
  for a conversation with none (`You are Qwen, created by Alibaba Cloud. You are a helpful assistant.` — this is why
  `ollama/Modelfile` bakes in that same line),
...
There is no trigger phrase and no system prompt to supply — send just the question as a user turn:

```python
messages = [{"role": "user", "content": "How much did I spend on groceries in 2024?"}]
...
`apply_chat_template` fills in Qwen's own default system line for you, matching training (see
[The system prompt](#the-system-prompt-trigger-or-baked-in)); don't override it with a custom one. Send the question
alone: the model is not trained on prompts that carry an account list (regenerate with `--schema-weights` if you want
that). Name exact accounts, payees, tags and currencies in the question when it matters; natural words such as
"groceries" are matched with a keyword regex.

## Running it in Ollama

[`ollama/Modelfile`](ollama/Modelfile) bakes in Qwen's default system line, a plain ChatML template and `temperature 0`,
so a client only has to send the question. Export your merged fine-tune as a GGUF (for example Q4_K_M), put the
...
The file is generated (`python scripts/make_modelfile.py --check` fails if it is stale), and `tests/test_modelfile.py`
checks that the Modelfile, the code and the training data all agree. Details that matter:
